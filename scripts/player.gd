...
#jump buffer
const JUMP_BUFFER_TIME_MAX := 0.15
...
func _physics_process(delta):
    ...
    # jump buffer and double jump input handling
    if Input.is_action_just_pressed("jump"):
        if is_on_floor():
            jump_buffer_time = JUMP_BUFFER_TIME_MAX
        elif state == PlayerState.JUMP and can_double_jump:
...
func _on_double_jump_effect_animation_finished():
    $SpriteHolder/DoubleJumpEffect.hide()
